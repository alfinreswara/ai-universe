...
export function searchCatalog(query:string):SearchResult[]{
 ...
 const items:SearchResult[]=[...providers.filter(p=>p.published).map(p=>({id:p.id,name:p.name,kind:'Provider' as const,subtitle:'Provider system'})),...families.map(f=>({id:f.id,name:f.name,kind:'Family' as const,subtitle:providers.find(p=>p.id===f.providerId)!.name,provider:f.providerId})),...models.filter(m=>m.published).map(m=>({id:m.id,name:m.name,kind:'Model' as const,subtitle:providers.find(p=>p.id===m.providerId)!.name,provider:m.providerId})),...capabilities.map(c=>({id:c.name,name:c.name,kind:'Capability' as const,subtitle:`${models.filter(m=>m.capabilities.includes(c.name)).length} models in this collection`}))];
